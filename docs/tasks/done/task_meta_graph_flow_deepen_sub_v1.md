...
| 字段 | 值 |
|------|-----|
| **task_slug** | `meta-graph-flow-deepen-sub` |
| **test_strategy** | `required` |
| **test_strategy_note** | `python3 tools/tech_graph/graph_yaml_compile.py --graph-id 10_flow_subagent` 必须成功。关账 graph:ci 归 W-close。 |
...
| **entry_invoke_30** | `docs/harness/invokes/by-task/meta-graph-flow-deepen/PROMPT_30_meta-graph-flow-deepen-sub.md` |
...
把 `10_flow_subagent` 从骨架/partial/skeleton 提升到 SPEC §4.2 **deep**。

加深重点：补 timeout / user-cancel / rate-limit 侧链（subagent-batch.ts）。TUI 仍可选消费。

## 2. 范围

- [x] 只编辑 `docs/_tech_graph/10_flow_subagent.graph.yaml`（可 compile 生成同名 `.md`）
- [x] 达到 D1–D8；自检写明锚覆盖率（path%、line%）
- [x] `graph_id` 保持 `10_flow_subagent`
...
- [x] D1–D8 自检表填齐
- [x] `compile --graph-id 10_flow_subagent` exit 0
- [x] 未改其它 yaml / 生产码
...
| D2 失败 | `[timeout]`（batch L627/L653）· user-cancel（L171/L553）· rate-limit requeue（L426）/ only-unfinished `[err]`（L394） |
| D3 锚 | 硬边 16：path **100%**（16/16）· line **100%**（16/16） |
| D4 无 TBD | 无 |
| D5 协议 | `?>` / `[ok]` / `[err]` / `[timeout]` / `::triggers` / `::yields` |
| D6 折叠 | 15 节点；ramp / capacity recovery / `startBtw` 折叠 |
| D7 真码 | path 均现存；line 未溢出 |
| D8 回归 | `python3 tools/tech_graph/graph_yaml_compile.py --graph-id 10_flow_subagent` exit 0 |

TUI 仍为可选 `::yields`。未 `--all` / 未 export / 未改生产码。

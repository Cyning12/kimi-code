...
| 字段 | 值 |
|------|-----|
| **task_slug** | `meta-graph-flow-deepen-mcp` |
| **test_strategy** | `required` |
| **test_strategy_note** | `python3 tools/tech_graph/graph_yaml_compile.py --graph-id 10_flow_mcp_tool` 必须成功。关账 graph:ci 归 W-close。 |
...
| **entry_invoke_30** | `docs/harness/invokes/by-task/meta-graph-flow-deepen/PROMPT_30_meta-graph-flow-deepen-mcp.md` |
...
把 `10_flow_mcp_tool` 从骨架/partial/skeleton 提升到 SPEC §4.2 **deep**。

加深重点：补 needs-auth / reconnect 侧链；transport 继续折叠。

## 2. 范围

- [ ] 只编辑 `docs/_tech_graph/10_flow_mcp_tool.graph.yaml`（可 compile 生成同名 `.md`）
- [ ] 达到 D1–D8；自检写明锚覆盖率（path%、line%）
- [ ] `graph_id` 保持 `10_flow_mcp_tool`
...
- [ ] D1–D8 自检表填齐
- [ ] `compile --graph-id 10_flow_mcp_tool` exit 0
- [ ] 未改其它 yaml / 生产码
...
| ID | 结果 | 说明 |
| --- | --- | --- |
| D1 主干 | pass | `mcp/index` → `connectAll` → `connectOne`/`createClient` → `connectAndDiscoverTools` → status → `connected` → `attachMcpTools` → `registerMcpServer` → `loopTools` → `callTool` 连续 |
| D2 失败 | pass | `[err]`：startup failed / unexpected close / OAuth `errorResult` / `reconnect` unknown·disabled；`[timeout]`：startup `withTimeout` · OAuth `complete` |
| D3 锚覆盖 | pass | 硬边 18/18 **path 100%** · **line 100%**（全边 22/22 同） |
| D4 无 TBD | pass | yaml 无 `TBD`；path 均现存 |
| D5 协议 | pass | `?>` / `[ok]` / `[err]` / `[timeout]` / `::triggers` / `::yields`；未发明 mark |
| D6 折叠 | pass | 15 节点；stdio/sse/http 仍折 `createClient`；OAuth begin/complete 折 `createMcpAuthTool`；无新 `10_flow_*` |
| D7 真码 | pass | 新增 `MCP_RECONN` → `reconnect()`；侧链锚 `auth-tool.ts` / `oauth/service.ts` / `connection-manager.ts` |
| D8 回归 | pass | `python3 tools/tech_graph/graph_yaml_compile.py --graph-id 10_flow_mcp_tool` exit 0 |

口径：硬边 = mark 非 `::*` 元关系。`graph_id` 仍为 `10_flow_mcp_tool`。未 `--all` / export / 改生产码。

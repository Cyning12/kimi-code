...
- `apps/kimi-web`: Vue browser client. Runtime talks to the local server over HTTP/WS; must not depend on `@moonshot-ai/agent-core`.
...
- `packages/protocol`: shared REST and WebSocket schema.
- `packages/server`: local REST+WS server that hosts agent-core and can serve kimi-web static assets.
- `packages/server-e2e`: wire E2E tests against the protocol.
